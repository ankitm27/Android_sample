...
public class OriginalSectionCellViewGroup extends FrameLayout
    ...
    public void bindSection(NexusWithImage item, int row, int column) {
        iv_image.setVisibility(column == 0 && item.resImages.size()==0 ? GONE : VISIBLE);
        if(iv_image.getVisibility()==VISIBLE) {
            iv_image.setImageResource(column > 0 && item.resImages.size() > 0 ? item.resImages.get(column - 1) : 0);
        }
