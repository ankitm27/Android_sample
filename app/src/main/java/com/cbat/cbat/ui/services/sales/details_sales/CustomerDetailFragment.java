...
//    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        GlobalClass.pdfDataTab="customerDetails";
        if (GlobalClass.customerFullList != null && !GlobalClass.customerFullList.isEmpty()) {
            Log.d(TAG, "Other Size >> " + GlobalClass.customerFullList.size());
                    ...
                    if (!set.getKey().equalsIgnoreCase("totalStateSales")) {
                        ...
                        for (int i = 0; i < dataMonthly.length(); i++) {
                            ...
                            if (contriFlag) {
                                ...
                                Log.d(TAG, "yVal >" + yVal);
                               // dataTable[j++] = String.valueOf(set.getKey());
                                String moneyString = formatter.format(Utility.decimal2PalceAsInput(Math.abs(yVal)));
                        ...
                        Log.d(TAG, "CutomerName >" +dataTable[0]);
                        ...
                        dataTable[j++]=dataCustomer.getJSONObject(0).getJSONObject("customer").getString("address")+"\n"+
                                "State:"+ dataCustomer.getJSONObject(0).getJSONObject("customer").getString("state")+"\n"+
                                "Credit Amount:"+ dataCustomer.getJSONObject(0).getJSONObject("customer").getString("creditAmount")+"\n"+
                                "Credit Peroid:"+dataCustomer.getJSONObject(0).getJSONObject("customer").getString("creditPeroid")+"\n"+
                                "GST:"+dataCustomer.getJSONObject(0).getJSONObject("customer").getString("partygstin");
                        Log.d(TAG, "j >" +j+ " << >> "+dataTable[j-1]);
            ...
            try {
                if (customerFullList != null && !customerFullList.isEmpty() && customerFullList.containsKey("totalStateSales")) {
                    ...
                    for (int i = 0; i < dataMonthly.length(); i++) {
                        ...
                        String moneyString1 = formatter.format(Utility.decimal2PalceAsInput(Math.abs(debitFloat)));
                        //dataTable[j++] = Utility.decimal2Palce(Math.abs(debitFloat));
                        dataTable[j++] =moneyString1;
